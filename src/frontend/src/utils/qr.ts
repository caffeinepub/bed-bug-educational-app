 ...
 */
export function buildQrImageUrl(data: string, size = 400): string {
  ...
  try {
    ...
    if (!response.ok) {
      return {
        success: false,
        error:
          "Failed to download QR code. The QR service may be unavailable. Please try again later.",
      };
    }
    ...
    return {
      success: false,
      error:
        "Failed to download QR code. Please check your internet connection and try again.",
    };
  }
}
