...
export function useZipCodeBoundary(zipCode: bigint | null, stateAbbr: string | null) {
  const { actor, isFetching } = useActor();
  ...
  return useQuery<ZipBoundaryCoordinate[] | null>({
    ...
    queryFn: async () => {
      ...
      try {
        console.log(`Fetching boundary for zip: ${zipCode}, state: ${stateAbbr}`);
        ...
        if (parsed.boundaryCoordinates && Array.isArray(parsed.boundaryCoordinates)) {
          console.log(`Successfully parsed ${parsed.boundaryCoordinates.length} boundary coordinates`);
